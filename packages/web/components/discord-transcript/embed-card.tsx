...
export function DiscordEmbedCard({ embed }: { embed: DiscordEmbed }) {
        ...
        {embed.fields && embed.fields.length > 0 && (
          <div className="mb-2 grid gap-2 sm:grid-cols-2">
            {embed.fields.map((field, i) => (
              <div key={i} className={field.inline === false ? "sm:col-span-2" : undefined}>
                <div className="text-xs font-semibold text-text-primary">{field.name}</div>
                <div className="whitespace-pre-wrap text-xs text-text-secondary">{field.value}</div>
              </div>
            ))}
          </div>
        )}
