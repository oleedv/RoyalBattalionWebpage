...
Each package is its own container. Railway builds from `packages/{web,api,lobby-service}/railway.json` (`restartPolicyType: ON_FAILURE`, 10 retries). A push to the deploy branch starts a build; there is no separate CI.
...
This repo is used by the Royal Battalion staff team. External PRs are not expected; internal changes should still be reviewable.

1. Branch from the current deploy line. Use a focused name (`feat/…`, `fix/…`).
...
6. Open a pull request against the deploy branch. Describe the user-visible change and any env or permission additions.
