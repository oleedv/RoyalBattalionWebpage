...
- **⚠️ Local-Windows build caveat (verified 2026-07-05):** `bun run build` does NOT complete on this Windows dev box — `next build` fails at its `standalone` output-tracing stage with `EPERM ... symlink` (Windows symlink creation needs elevation/Developer Mode). This is post-compile/post-typecheck and pre-existing (unrelated to any birthday code); it builds fine on Linux/Railway CI. **For LOCAL verification of webpage tasks, gate on per-package typecheck instead:** `cd packages/web && bunx tsc --noEmit` (expect 0 errors) and `cd packages/shared && bunx tsc --noEmit` (expect 0), plus the API rule below. The web-package typecheck that `next build` runs succeeds before the symlink step, so a clean `packages/web` tsc is an equivalent gate.
