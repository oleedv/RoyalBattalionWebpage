...
export async function syncAllUserRoles(): Promise<number> {
  ...
  if (!botToken || !guildId) {
    console.warn(`[role-sync] Skipping: botToken=${botToken ? "set" : "MISSING"}, guildId=${guildId || "MISSING"}`);
    return 0;
  }

  const guildMembers = await fetchAllGuildMembers(botToken, guildId);
  console.log(`[role-sync] Fetched ${guildMembers.length} guild members`);
  ...
  console.log(`[role-sync] Mapped Discord roles in DB: ${discordRoles.map((r) => `${r.name}(${r.discordRoleId})`).join(", ") || "NONE"}`);

  // Get all users with their current role assignments
  const users = await prisma.user.findMany({
    include: { roles: true },
  });
  console.log(`[role-sync] Processing ${users.length} users from database`);
  ...
  for (const user of users) {
    ...
    if (!memberRoles) {
      console.log(`[role-sync] User ${user.discordName}(${user.discordId}) not found in guild members, skipping`);
      continue;
    }
    ...
    console.log(`[role-sync] User ${user.discordName}: Discord roles=${JSON.stringify(memberRoles)}, mapped DB roles=${expectedRoleIds.size}, current DB roles=${currentRoleIds.size}`);
    ...
    console.log(`[role-sync] Updating roles for ${user.discordName}: ${currentRoleIds.size} -> ${expectedRoleIds.size} roles`);
